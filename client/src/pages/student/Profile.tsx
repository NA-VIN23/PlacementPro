            ...
            <div className="relative group">
                ...
                <div className="relative px-8 pb-4 flex flex-col md:flex-row items-end -mt-12 gap-6">
                    ...
                    <div className="flex gap-3 pb-2">
                        {isEditing ? (
                            <>
                                <button onClick={() => setIsEditing(false)} className="px-4 py-2 bg-slate-100 text-slate-700 font-medium rounded-lg hover:bg-slate-200 flex items-center gap-2">
                                    <X className="w-4 h-4" /> Cancel
                                </button>
                                <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 shadow-sm shadow-green-500/30 flex items-center gap-2 disabled:opacity-50">
                                    <Save className="w-4 h-4" /> {isSaving ? 'Saving...' : 'Save'}
                                </button>
                            </>
                        ) : (
                            <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-brand-600 text-white font-medium rounded-lg hover:bg-brand-700 shadow-sm shadow-brand-500/30 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            {isEditing ? (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input name="linkedin" value={profileData.linkedin} onChange={handleChange} placeholder="LinkedIn URL" className="flex-1 text-sm px-3 py-1.5 border border-slate-200 rounded-lg" />
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Github className="w-4 h-4 text-slate-800" />
                                        <input name="github" value={profileData.github} onChange={handleChange} placeholder="GitHub URL" className="flex-1 text-sm px-3 py-1.5 border border-slate-200 rounded-lg" />
                                    </div>
                                </div>
                            ) : (
                                <div className="flex gap-4">
                                    {(profileData.linkedin || profileData.github) ? (
                                        <>
                                            {profileData.linkedin && <a href={`https://${profileData.linkedin}`} target="_blank" className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100"><Linkedin className="w-5 h-5" /></a>}
                                            {profileData.github && <a href={`https://${profileData.github}`} target="_blank" className="p-2 bg-slate-100 text-slate-800 rounded-lg hover:bg-slate-200"><Github className="w-5 h-5" /></a>}
                                        </>
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {profileData.skills.map(skill => (
                                <span key={skill} className={cn("px-3 py-1 bg-slate-50 text-slate-600 text-sm font-medium rounded-md border border-slate-100 flex items-center gap-1", isEditing && "pr-1.5")}>
                                    {skill}
                                    {isEditing && <button onClick={() => removeSkill(skill)} className="ml-1 p-0.5 hover:bg-red-100 rounded text-red-500"><X className="w-3 h-3" /></button>}
                                </span>
                            ))}
                        </div>
                        {isEditing && (
                            <div className="mt-4 flex gap-2">
                                <input value={newSkill} onChange={(e) => setNewSkill(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSkill()} placeholder="Add skill..." className="flex-1 text-sm px-3 py-2 border border-slate-200 rounded-lg" />
                                <button onClick={addSkill} className="px-4 py-2 bg-brand-600 text-white text-sm font-medium rounded-lg">Add</button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        ...
                        {isEditing ? (
                            <textarea name="bio" value={profileData.bio} onChange={handleChange} rows={4} className="w-full text-sm px-3 py-2 border border-slate-200 rounded-lg" placeholder="Write something about yourself..." />
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {isEditing && <button onClick={() => addItem('internships', { role: '', company: '', duration: '', description: '' })} className="text-sm text-brand-600 font-medium hover:underline flex items-center gap-1"><Plus className="w-4 h-4" /> Add</button>}
                        </div>
                        <div className="space-y-6">
                            {profileData.internships.map((intern, i) => (
                                <div key={i} className="group relative pl-4 border-l-2 border-slate-100 hover:border-brand-200 transition-colors">
                                    {isEditing ? (
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {isEditing && <button onClick={() => addItem('projects', { title: '', description: '', techStack: '', link: '' })} className="text-sm text-brand-600 font-medium hover:underline flex items-center gap-1"><Plus className="w-4 h-4" /> Add</button>}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {profileData.projects.map((proj, i) => (
                                <div key={i} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-slate-50/50">
                                    {isEditing ? (
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {isEditing && <button onClick={() => addItem('education', { degree: '', school: '', year: '', score: '' })} className="text-sm text-brand-600 font-medium hover:underline flex items-center gap-1"><Plus className="w-4 h-4" /> Add</button>}
                        </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            {isEditing && <button onClick={() => addItem('certifications', { title: '', issuer: '' })} className="text-sm text-brand-600 font-medium hover:underline flex items-center gap-1"><Plus className="w-4 h-4" /> Add</button>}
                        </div>
